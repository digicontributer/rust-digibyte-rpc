...
TESTDIR=/tmp/rust_digibyte_rpc_test
...
BLOCKFILTERARG=""
if digibyted -version | grep -q "v0\.\(19\|2\)"; then
    BLOCKFILTERARG="-blockfilterindex=1"
fi

FALLBACKFEEARG=""
if digibyted -version | grep -q "v0\.2"; then
    FALLBACKFEEARG="-fallbackfee=0.00001000"
fi

digibyted -regtest $BLOCKFILTERARG $FALLBACKFEEARG \
    -datadir=${TESTDIR}/2 \
